...
%%%
%%% @end
%%% Created : 22. 十二月 2015 10:52 AM
%%%-------------------------------------------------------------------
-module(distributed_proxy_ring_events).
-author("zy").
...
-export([
    start_link/0,
    add_handler/2, add_sup_callback/1,
    ring_update/1, ring_sync_update/1
]).
...
add_sup_callback(Fn) when is_function(Fn) ->
    gen_event:add_sup_handler(?SERVER, {?MODULE, make_ref()}, [Fn]).

ring_update(Ring) ->
    gen_event:notify(?SERVER, {ring_update, Ring}).

ring_sync_update(Ring) ->
    gen_event:sync_notify(?SERVER, {ring_update, Ring}).
